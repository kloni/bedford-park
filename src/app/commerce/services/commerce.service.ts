...
export class CommerceService {
    ...
    readonly catLink:string;
    readonly plpLink:string;
    ...
    getCategoryNavigation(): Promise<any> {
        return new Promise((resolve) => {
            const updateCats = (cat) => {
                cat.id = cat.uniqueID;
                if(cat.catalogGroupView) {
                    cat.route = this.catLink;
                    cat.routeParam = {'categoryIdentifier': cat.identifier};
                    cat.url = cat.route;
                    cat.children = cat.catalogGroupView.map(updateCats);
                } else {
                    cat.route = this.plpLink;
                    cat.routeParam = {'categoryId': cat.uniqueID};
                    cat.url = cat.route;
                    cat.children = [];
            ...
            this.getItems(null).then((cat) => {
                resolve(cat.map(updateCats));
            })
        })
    }
