...
export class AccountTransactionService {
	...
	updateCurrentUserPersonalInfo(user: any): Promise<HttpResponse<any>> {
		...
		if (user[Constants.DIGITAL_ANALYTICS_CONSENT]) {
			params.body[Constants.DIGITAL_ANALYTICS_CONSENT] = user[Constants.DIGITAL_ANALYTICS_CONSENT];
		}
		if (user[Constants.MARKETING_TRACKING_CONSENT]) {
			params.body[Constants.MARKETING_TRACKING_CONSENT] = user[Constants.MARKETING_TRACKING_CONSENT];
		}
	...
	updateCurrentUserPassword(user: any): Promise<HttpResponse<any>> {
		const params = {
			'storeId': this.storefrontUtils.commerceStoreId,
			'body': {
				"logonId": user.logonId,
				"xcred_logonPasswordOld": user.curPass,
				"logonPassword": user.newPass,
				"xcred_logonPasswordVerify": user.cnfPass,
				"resetPassword": "true"
			}
		};

		return this.personService.updatePersonOnUserRegistrationUpdate(params, undefined).toPromise().then(res => {
			this.logger.info( this.constructor.name + " updateCurrentUserPassword: %o", res );
			return res;
		});
	}
}
