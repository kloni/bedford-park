...
@Injectable()
export class ProductListingInfiniteTransactionService {
	...
	getProductsByQuery(urlParams: any, facetParams?: any, refreshFacets?: boolean, sendDATag?: boolean, position: number = 0) {
				...
				const prodObservable = productObservable.subscribe(
					response => {
						if (response && response.body) {
								...
								this.loadProductsAndFacets(response.body, facet, position);
	...
	loadProductsAndFacets(productresults: any, facetToUpdate?:  any, position: number = 0) {
		...
		if (this.productSource) {
            if (position < 0) {
                products.globalResults = products.globalResults.concat(this.productSource['globalresults']);
            } else if (position > 0) {
                products.globalResults = this.productSource['globalresults'].concat(products.globalResults);
            }
        }
	...
	*/
	...
	fetchPage(pageNumber: number, position: number = 0) {
		this.activeFacets['pageNumber'] = pageNumber;
		this.getProductsByQuery(this.urlParams, this.activeFacets, undefined, true, position);
	}
