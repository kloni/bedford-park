...
export class ConfigurationService extends TransactionService {
    ...
    findByConfigurationId(parameters: any, headers ? : any, url ? : string): Observable < HttpResponse < any >> {
        ...
        if (this.getStorefrontUtils().useMocks || useMocks) {
            ...
            requestUrl = 'mocks/commerce/transaction' + path + fileNameSeparator + testGroup + '.findByConfigurationId.mocks.json';
        }
        let form = {};
        let body = {};
        ...
        requestUrl = requestUrl.replace('{configurationId}', parameters['configurationId']);
