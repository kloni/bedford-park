...
export class PaymentInstructionService extends TransactionService {
    ...
    addPaymentInstruction(parameters: any, headers ? : any, url ? : string): Observable < HttpResponse < any >> {
        ...
        if (this.getStorefrontUtils().useMocks || useMocks) {
            ...
            requestUrl = 'mocks/commerce/transaction' + path + fileNameSeparator + testGroup + '.addPaymentInstruction.mocks.json';
        }
        let form = {};
        let body = {};
     ...
     * `@param {any} parameters` have following properties:
     ** `@property {string} storeId (required)` The child property of `Parameters`.The store identifier.
     ** `@property {string} paymentInstruction_id (required)` The child property of `Parameters`.Payment instruction identifier to delete.
     ** `@property {string} responseFormat ` The response format. If the request has an input body, that body must also use the format specified in "responseFormat". Valid values include "json" and "xml" without the quotes. If the responseFormat isn't specified, the "accept" HTTP header shall be used to determine the format of the response. If the "accept" HTTP header isn't specified as well, the default response format shall be in json.
     */
    deletePaymentInstruction(parameters: any, headers ? : any, url ? : string): Observable < HttpResponse < any >> {
        ...
        if (this.getStorefrontUtils().useMocks || useMocks) {
            ...
            let fileNameSeparator = testGroup === "" ? "" : ".";
            requestUrl = 'mocks/commerce/transaction' + path + fileNameSeparator + testGroup + '.deletePaymentInstruction.mocks.json';
        }
        let form = {};
        let body = {};
        let header: HttpHeaders;
        ...
        requestUrl = requestUrl.replace('{paymentInstruction_id}', parameters['paymentInstruction_id']);

        if (parameters['paymentInstruction_id'] === undefined) {
            throw new Error("Request '/store/{storeId}/cart/@self/payment_instruction/{paymentInstruction_id}' missing required parameter paymentInstruction_id");
        }
