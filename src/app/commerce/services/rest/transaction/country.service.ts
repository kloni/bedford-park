...
export class CountryService extends TransactionService {
    ...
    findCountryStateList(parameters: any, headers ? : any, url ? : string): Observable < HttpResponse < any >> {
        ...
        if (this.getStorefrontUtils().useMocks || useMocks) {
            ...
            requestUrl = 'mocks/commerce/transaction' + path + fileNameSeparator + testGroup + '.findCountryStateList.mocks.json';
        }
        let form = {};
        let body = {};
        ...
        if (parameters['countryCode'] !== undefined) {
            queryParameters = serviceUtils.setQueryParam(queryParameters, 'countryCode', parameters['countryCode']);
        }
