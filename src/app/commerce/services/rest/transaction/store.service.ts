...
export class StoreService extends TransactionService {
     ...
     */
    getStoreEnabledFeaturesList(parameters: any, headers ? : any, url ? : string): Observable < HttpResponse < any >> {
        let useMocks = false;
        ...
        let path = '/store/{storeId}/features';
        ...
        if (this.getStorefrontUtils().useMocks || useMocks) {
            ...
            requestUrl = 'mocks/commerce/transaction' + path + fileNameSeparator + testGroup + '.getStoreEnabledFeaturesList.mocks.json';
        }
        let form = {};
        let body = {};
        ...
        if (parameters['storeId'] === undefined) {
            throw new Error("Request '/store/{storeId}/features' missing required parameter storeId");
        }
